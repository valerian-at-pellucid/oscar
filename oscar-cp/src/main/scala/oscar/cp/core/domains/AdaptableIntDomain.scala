 ...
 */
class AdaptableIntDomain(context: ReversibleContext, val minValue: Int, val maxValue: Int) extends SparseIntDomain {
  
  // Reversible domain representation
  private val domain: ReversiblePointer[IntervalIntDomain] = {
    new ReversiblePointer[IntervalIntDomain](context, new BoundDomain(context, minValue, maxValue))
  }
  ...
  override def removeValue(value: Int): CPOutcome = {
    ...
    if (dom.isInstanceOf[SparseIntDomain]) {
      dom.asInstanceOf[SparseIntDomain].removeValue(value)
    }
    else {
