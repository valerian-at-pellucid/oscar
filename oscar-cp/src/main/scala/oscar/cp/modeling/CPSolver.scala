...
class CPSolver() extends CPStore() {
  ...
  def addDecisionVariables(x: Iterable[CPVarInt]): Unit = x.foreach(decVariables += _)
  
  def addDecisionVariables(x: CPVarInt*): Unit = x.foreach(decVariables += _)
  
  private def recordSol(): Unit = {
    lastSol = new CPSol(decVariables.toSet)
  }
  
  def obj(objVar: CPVarInt): CPObjectiveUnit = objective(objVar)
