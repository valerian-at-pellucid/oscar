...
object Exodus {
  ...
  def main(args: Array[String]) {
    ...
    // variables
    //
    val story = Array.fill(n)(CPVarInt(cp, 0 to n-1))
    ...
    val country = Array.fill(n)(CPVarInt(cp, 0 to n-1))
    ...
    val age = Array.fill(n)(CPVarInt(cp, Array(3,5,7,8,10)))
    ...
    cp.solve subjectTo {
      ...
      cp.add(age(yemen) < age(ethiopia))
      cp.add(age(morocco) == age(ted) + 3)
      cp.add(age(sammy) == age(mosessyouth) + 3)
      cp.add(captivity == carl)
      cp.add(age(tencommandments) == 5)
      cp.add( 
