 ...
 */
class TestQuad extends FunSuite with ShouldMatchers {
...
*/
  test("Test QuadTree 11") {
    ...
    for (i <- 0 until 1000) {
      ...
      val y = -x+1000+rand.nextInt(200) - ((x-500).abs)
      
      
      tree.insert(Array(x,y))
    }
    val s = tree.toSet.toArray 
    println("x=c"+s.map(v => v(0)).mkString("(",",",")"))
    println("y=c"+s.map(v => v(1)).mkString("(",",",")"))
