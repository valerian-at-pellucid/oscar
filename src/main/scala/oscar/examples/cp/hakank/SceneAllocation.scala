...
/*
  ...
  Thanks to Pierre Schaus for the labeling and speedup.
...
object SceneAllocation {
  ...
  def main(args: Array[String]) {
    ...
    val which = for(a <- ACTORS) yield (for(s <- SCENES; 
                                            a2 <- appears(s) if a == a2) yield s);

    //
    // variables
    //
    val shoot = Array.fill(maxScene)(CPVarInt(cp, 0 until maxDay)) // the day a particular scene is shooted
    ...
    cp.minimize(cost) subjectTo {
      ...
      cp.add(gcc(shoot, 0 until maxDay, 0, 5), Strong) 

      cp.add(shoot(0) == 0)

    } exploration {

      // This labeling is from the mind of Pierre Schaus.
