 ...
 * 
 */
class VisualTour(nodesPos: Array[(Int, Int)], scale: Boolean) extends VisualDrawing(true, true, scale) {
...
object VisualTour {
  ...
  def apply(nodes: Array[(Int, Int)], scale: Boolean = true): VisualTour = {
    new VisualTour(nodes, scale)
  }
}