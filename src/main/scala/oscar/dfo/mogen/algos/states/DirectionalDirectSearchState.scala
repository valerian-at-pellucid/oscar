...
class DirectionalDirectSearchState[E <% Ordered[E]](initPoint: MOOPoint[E], var stepSizes: Array[Double], var basisSize: Int) extends ComparativeAlgorithmState[E] {
  ...
  def getNewState(newBestPoint: MOOPoint[E], comparator: MOOComparator[E]): ComparativeAlgorithmState[E] = {
    DirectionalDirectSearchState(newBestPoint, stepSizes, basisSize)
  }
  
  def getNewPoint(directionIndex: Int, evaluator: MOEvaluator[E], feasibleReg: FeasibleRegion): MOOPoint[E] = {
    val randDirection = ArrayUtils.normalize(Array.tabulate(initPoint.nbCoordinates)(i => RandomGenerator.nextDouble))
    val newCoordinates = Array.tabulate(bestPoint.nbCoordinates)(i => bestPoint.coordinates(i) + stepSizes(i) * randDirection(i))
...
object DirectionalDirectSearchState {
  ...
  def apply[E <% Ordered[E]](initPoint: MOOPoint[E], stepSizes: Array[Double], basisSize: Int) = new DirectionalDirectSearchState(initPoint, stepSizes, basisSize)
  
  def apply[E <% Ordered[E]](initPoint: MOOPoint[E], stepSizeIntervals: Array[(Double, Double)]): ComparativeAlgorithmState[E] = {
    val basisSize = math.max(initPoint.nbCoordinates / 2, 2).toInt
   ...
   DirectionalDirectSearchState(initPoint, stepSizes, basisSize)
  }
}