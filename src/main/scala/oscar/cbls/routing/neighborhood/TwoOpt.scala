 ...
 */
object TwoOpt extends Neighborhood with SearchEngineTrait {
        ...
        for (
          ...
          checkEncodedMove(moveAcceptor(startObj), !returnMove, vrp) match {
            case (true, newObj: Int) => { //this improved
              if (returnMove) return MoveFound(TwoOpt(fstEdgeStartPoint, sndEdgeStartPoint, newObj, vrp))
...
case class TwoOpt(
  ...
  sndEdgeStartPoint: Int,
  objAfter: Int,
  vrp: VRP with MoveDescription) extends Move(objAfter, vrp) {
  // overriding methods
  override def encodeMove() {
    TwoOpt.encode(fstEdgeStartPoint, sndEdgeStartPoint, vrp)
  }
