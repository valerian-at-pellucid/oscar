...
import newVRP.{MoveDescription, VRP, PositionInRouteAndRouteNr}
import oscar.cbls.routing.neighborhood.newneighborhood.Move
import oscar.cbls.search.SearchEngine
import oscar.cbls.modeling.Algebra._
import oscar.cbls.routing.model._
import newVRP.MoveDescription
import oscar.cbls.search.SearchEngine
...
object OnePointMove extends SearchEngine{
  ...
  def getFirstImprovingMove(vrp:VRP with ObjectiveFunction with PositionInRouteAndRouteNr,
                            relevantNeighborhoodOfNode:(Int=> Iterable[Int])= null, nodeIterator:Iterable[Int]):OnePointMove =
    findMove(true,vrp,if(relevantNeighborhoodOfNode == null)_=>vrp.Nodes else relevantNeighborhoodOfNode,nodeIterator)
  ...
  private def findMove(FirstImprove:Boolean,vrp:VRP with ObjectiveFunction with PositionInRouteAndRouteNr,
                       relevantNeighborhoodOfNode:Int=> Iterable[Int],nodeIterator:Iterable[Int]):OnePointMove = {
    var BestObj:Int = vrp.ObjectiveVar.value
    var move:((Int, Int)) = null

    for (beforeMovedPoint <- nodeIterator if vrp.isRouted(beforeMovedPoint)){
 ...
 * @param vrp the given VRP problem.
 */
case class OnePointMove(predOfMovedPoint:Int, PutAfter:Int, override val objAfter:Int, override val vrp:VRP with MoveDescription, val restartPoint:Int) extends Move(objAfter,vrp){
  // overriding methods
  override def comit() {
    val s = vrp.cutNodeAfter(predOfMovedPoint)
    vrp.insert(s,PutAfter)
  }
