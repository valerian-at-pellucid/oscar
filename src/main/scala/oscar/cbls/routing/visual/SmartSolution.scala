...
class SmartSolution() extends JFrame{
  ...
  def setColorToRoute(l:VisualLine ,i:Int){
    l.setOuterCol(PanelVRP.PanelVRP.colorsManagement(i+1))
  }
  ...
  def displayNodes(){
    ...
    for(i <- 0 until nodes.length){
      ...
      if (i<PanelVRP.vrpModel.V)
        new VisualCircle(mapPanel, t.long,t.lat,10,Color.blue).setInnerCol(PanelVRP.PanelVRP.colorsManagement(i+1))
      else
