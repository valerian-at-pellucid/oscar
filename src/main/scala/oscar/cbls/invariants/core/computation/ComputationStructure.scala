 ...
 * see methods getSolution and restoreSolution in [[invariants.core.computation.Model]]
 */
...
class Event(v:Variable, w:Variable, ModifiedVars:Iterable[Variable]) extends Invariant{
  ...
  override def performPropagation(){
    ...
    if(intintaction!=null){
      oldIntv = v.asInstanceOf[IntVar]
      oldIntw = w.asInstanceOf[IntVar]
    }
    if (intsetintsetaction!=null){
      oldIntSetv = v.asInstanceOf[IntSetVar]
      oldIntSetw = w.asInstanceOf[IntSetVar]
    }
    if (intsetintaction!=null){
      oldIntSetv = v.asInstanceOf[IntSetVar]
      oldIntw = w.asInstanceOf[IntVar]
...
/**an intvar is a variable managed by the [[invariants.core.computation.Model]] whose type is integer.
 *
 * @param model is the model in s-which the variable is declared, can be null if the variable is actually a constant, see [[invariants.core.computation.IntConst]]
...
/**An IntSetVar is a variable managed by the [[invariants.core.computation.Model]] whose type is set of integer.
 * @param model is the model in s-which the variable is declared, can be null if the variable is actually a constant, see [[invariants.core.computation.IntSetConst]]
...
/**this is a very experimental feature, so use it at you own risk
 * if a variable is created with this trait,
 * it will automatically perform propagation under if the model is not closed yet,
 * and if no invariant is dynamically listening to this variable.
 * both conditions must be enforced together.
 */
trait AutoPropagateBeforeStart extends Variable{
  override def notifyChanged(){
    //TODO: this should not be put here!!! any const can have a null model, in which case, they must be propagated asap.
    if (this.model == null ||(!this.model.isClosed && this.getDynamicallyListeningElements.isEmpty)){
      this.performPropagation()
    }else{
      super.notifyChanged()
    }
  }
...
abstract class IntInvariant extends Invariant{
  ...
  implicit def toIntVar:IntVar = {
    val a = new IntVar(model,MyMin,MyMax,0,this.getClass.getSimpleName) with AutoPropagateBeforeStart
   ...
   * see [[invariants.core.computation.IdentityInt]] and [[invariants.core.computation.IdentityIntSet]]
