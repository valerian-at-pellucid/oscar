...
object TSPDemo {
  ...
  def main(args: Array[String]) {
    ...
    val obj = f.createFrame("TSP Objective Function")
    obj.add(plot)
    obj.pack()
    // creates the tour visu and place it into the frame
    val map = new VisualMap();
    map.viewer.setZoom(13)
    map.viewer.setPreferredSize(new java.awt.Dimension(700, 450));
    val tour = f.createFrame("TSP Tour")
    tour.add(map)
    tour.pack()
    //f.pack()
    // ------------------------------------------
    ...
    val countries = Array(("Albania", 41.153332,20.168331),
    					  ...
    					  ("United Kingdom", 55.378051,-3.435973)).map(t => new Location(t._2,t._3));
