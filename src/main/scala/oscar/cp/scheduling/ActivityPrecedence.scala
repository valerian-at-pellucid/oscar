...
private object SBS extends PrecedenceType { override def toString = "End before Start"}
...
class ActivityPrecedence(act1 : Activity, act2 : Activity, pType : PrecedenceType) {
	...
	def withDelay(delay : Int) = {
		...
		pType match {
			...
			case _   => throw new InvalidParameterException(pType + " is no a valid precedence type.")
		}
	}
}
