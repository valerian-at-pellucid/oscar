 ...
 */
object ChemicalTankerMO extends App {
    ...
    f.createFrame("Objectives").add(paretoPlot)
    ...
    // optimum sol for the chemical4.xml instance
    val optSol = Array((11,4611),(10,4950),(9,5074),(8,5115),(7,5153),(6,5154),(5,5135),(4,5132),(3,4351),(2,3098),(1,1549))
    for ((k,c) <- optSol) {
      paretoPlot.insert(c,k,1)
    }
    ...
    val freeSpaceNeg = -load(0)
    val nbFreeTanksNeg = -card(0)
    cp.recordNonDominatedSolutions = true
    cp.addDecisionVariables(cargo)
    cp.silent = true
    
    val t0 = System.currentTimeMillis()
    def printsol() {
      println(cp.nonDominatedSolutions.map(s => (-s(nbFreeTanksNeg),-s(freeSpaceNeg))).mkString(","))
    }
    //cp.timeLimit = 300
    cp.paretoMinimize(freeSpaceNeg,nbFreeTanksNeg) 
    //cp.minimize(freeSpaceNeg,nbFreeTanksNeg) 
    cp.subjectTo {
      ...
      println(freeSpaceNeg+" "+nbFreeTanksNeg)
      println("solution found at time:"+(System.currentTimeMillis()-t0))
      printsol()
    } exploration {
      //cp.binaryFirstFail(cargo,_.randomValue)
      ...
      //println("---")
      //println(freeSpace.value+" "+nbFreeTanks.value)
      //println(freeSpaceNeg.value+" "+nbFreeTanksNeg.value)
      paretoPlot.insert(freeSpace.value, nbFreeTanks.value)
      //println("#non dominated:"+cp.nonDominatedSolutions.size)
    } run(1)
    printsol()
    
    
    var i = 0
    while(paretoPlot.paretos(0) != paretoPlot.paretos(1)) {
      val sol = randomSol()
      paretoPlot.highlight(sol(freeSpaceNeg),sol(nbFreeTanksNeg))
      val toKeep = cargo.size/2
      cp.runSubjectTo(Int.MaxValue, 500) {
        if (i % 2 == 0) {
          cp.objective.diversify()  
        } else {
          cp.objective.intensify(sol,freeSpaceNeg)
        }
        val fixed = (for (i <- 1 to toKeep) yield rnd.nextInt(cargo.size)).toSet
        cp.post(fixed.map(i => cargo(i) == sol(cargo(i))))
      }
      if (i%100 == 0) {
        println("nb restarts:"+i)
        printsol()
      }
      i += 1
    }
    println("nb restarts:"+i)
    printsol()
    println("solution optimal found at time:"+(System.currentTimeMillis()-t0))
    
    def randomSol(): CPSol = {
      val sols = (for (x <- cp.nonDominatedSolutions) yield x).toArray
      sols(rnd.nextInt(sols.size))
    }
