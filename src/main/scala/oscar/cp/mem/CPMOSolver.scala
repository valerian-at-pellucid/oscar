...
class CPMOSolver(nObj : Int) extends Store() {
	
	// The approximation of the front of Pareto
	private val paretoFront: ParetoSet = new ParetoSet(nObj)

	// This array contains the variables involved in a solution
	private var vectorSol: Array[CPVarInt] = null

	// Current solution to relax
	def currentSol = currentPoint.solution
	
	private var currentPoint: ParetoPoint = null

	// Define the variables involved in a solution
	def defineSolution(sol: Array[CPVarInt]) { vectorSol = sol }

	// Builds a solution from the vector
	def buildSol: Array[Int] = vectorSol.map(v => v.value)
	...
	def minimize(obj: CPVarInt): CPMOSolver = {
		stateObjective = Unit => {
	...
	def minimize(objectives: CPVarInt*): CPMOSolver = {
		stateObjective = Unit => {
	...
	def maximize(obj: CPVarInt): CPMOSolver = {
		stateObjective = Unit => {
	...
	def solve(): CPMOSolver = {
		solveOne = true
		this
	}

	def solveAll(): CPMOSolver = {
		solveOne = false
		this
	}

	def subjectTo(constraintsBlock: => Unit): CPMOSolver = {
		try {
			constraintsBlock
		}
		catch {
	...
	override def exploration(block: => Unit @suspendable): Unit = {
		...
		if (vectorSol == null) 
			throw new Exception("The variables involved in a solution are not defined")
					...
					if (relaxation) {
						...
						for (i <- 0 until nObj) {		
							if (i == objective.currentObjectiveIdx)
								objective.bounds(i) = currentPoint(i)-1
							else
								objective.bounds(i) = currentPoint.nextValue(i)
						}
						
						relax()
					}
					
					// If the relaxation is ok
					if (!isFailed()) {
						...
						reset {
							...
							if (!isFailed()) {
								...
								// Build a new pareto point
								val values = Array.tabulate(nObj)(i => objective.bestObjs(i))
								val p = paretoFront.buildNewPoint(values, buildSol)
								
								paretoFront add p
								
								// Next point
								currentPoint = paretoFront.nextPoint
								// Reset objective
								objective.currentObjective = 0
							}
							
							else if (objective.nextObjective()) 
								currentPoint = paretoFront.nextPoint
						}
						
						// Let's go, unless the user decided to stop
						if (!sc.exit) sc.explore()
					}
				}
...
object CPMOSolver {
	...
	/** Creates a new CP Solver
	  */
	def apply(d : Int): CPMOSolver = new CPMOSolver(d)
}
