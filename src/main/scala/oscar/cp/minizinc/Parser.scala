...
//		      	  case _ => 
//		      	    addCPVarInt(ann, id, Set[Int](), false)
...
//			        // need to post a constraint obout eq on set
	  ...
	  e match {
          case Some("="~assign) =>
            assign match {
              ...
              case _ => 
		        addCPVarSet(ann, id, s)
		        // TODO : need to post a constraint obout eq on set
		        // cp.add(getCPVarSetFromString(id) == getCPVarSet(assign))
            }
          case None =>
	        addCPVarSet(ann, id, s)
