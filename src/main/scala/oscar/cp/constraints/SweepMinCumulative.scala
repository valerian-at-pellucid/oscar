...
class SweepMinCumulative(starts: Array[CPVarInt], ends: Array[CPVarInt], durations: Array[CPVarInt], demands: Array[CPVarInt], resources: Array[CPVarInt], capacity: CPVarInt, id: Int) extends Constraint(starts.head.store, "MinSweepCumulative") {
  ...
  protected val eventPointSeries: Array[Event] = new Array[Event](nTasks * 5)
   ...
   *  @param $idOfT
   */
  protected def generateCheck(t: Int) {
   ...
   *  @param $idOfT
   */
  protected def generateProfileBad(t: Int): Boolean = {
   ...
   *  @param $idOfT
   */
  protected def generateProfileGood(t: Int): Boolean = {
   ...
   */
  protected def consistencyCheck: Boolean = (nCurrentTasks > 0 && consSumHeight < capacity.min)
   ...
   */
  protected def mandatoryCheck(t: Int): Boolean = (nCurrentTasks != 0 && (consSumHeight - consContrib(t)) < capacity.min)
   ...
   */
  protected def forbidenCheck(t: Int): Boolean = (consSumHeight - consContrib(t) + demands(t).max < capacity.min)
