 ...
 * 
 */
class MaxMultiCumulative (cp: CPSolver, tasks : Array[CumulativeActivity], limits : Array[Int], minCumulative : Boolean) extends Constraint(tasks(0).getMachines.getStore(), "MaxMultiCumulative") {
	
	//val nTasks    = tasks.size
	val nResource = limits.size
	
	val Tasks     = 0 until tasks.size
	val Resource  = 0 until nResource
	...
	val inQueue : Array[Boolean] = new Array(limits.size)
	val checkQueue : Queue[Int] = Queue()
	...
	def generateEventPointSeries(r : Int) : Boolean = {
		...
		for (i <- 0 until tasks.size) {
			...
			if (tasks(i).hasCompulsoryPart && tasks(i).getMachines.isBoundTo(r)) {
				...
				// Check
				if (tasks(i).getMaxResource < limits(r)) {
	...
	def sweepAlgorithm(r : Int) : Tuple2[Boolean, CPOutcome] = {
		...
		while (event != null) {
			...
			if (!event.isPruningEvent) {
				...
				if (d != event.date) {
					// Consistency check
					if (sumHeight > limits(r)) return (change, CPOutcome.Failure)
		...
		// Consistency check
		if (sumHeight > limits(r)) return (change, CPOutcome.Failure) // TODO
		// Pruning
		res = prune(r, d, d)
		change |= res._1
		if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
	...
	def prune(r : Int, low : Int, up : Int) : Tuple2[Boolean, CPOutcome] = {
		...
		while (!it.isEmpty) {
			...
			if (change) {
				for(i <- tasks(t).getMachines.getMin to tasks(t).getMachines.getMax) {
					if (tasks(t).getMachines.hasValue(i) && !inQueue(i)) {
						inQueue(i) = true
						checkQueue.enqueue(i)
					}
				}
	...
	def pruneMandatory(t : Int, r : Int, low : Int, up : Int) : Tuple2[Boolean, CPOutcome] = {
		...
		// Consistency check
		if ((sumHeight - contribution(t)) <= limits(r)) { // TODO
			return (change, CPOutcome.Suspend)
		}
		
		// Fix the activity to the machine r and check consistency
		res = MaxMultiCumulative.fixVar(tasks(t).getMachines, r)
		change |= res._1
		if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
		
		// Adjust the EST of the activity and check consistency
		res = MaxMultiCumulative.adjustMin(tasks(t).getStart, up - tasks(t).getMaxDuration + 1)
		change |= res._1
		if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
		
		// Adjust the LST of the activity and check consistency
		res = MaxMultiCumulative.adjustMax(tasks(t).getStart, low)
		change |= res._1
		if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
		
		// Adjust the LCT of the activity and check consistency
		res = MaxMultiCumulative.adjustMax(tasks(t).getEnd, low + tasks(t).getMaxDuration)
		change |= res._1
		if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
		
		// Adjust the ECT of the activity and check consistency
		res = MaxMultiCumulative.adjustMin(tasks(t).getEnd, up + 1)
		change |= res._1
		if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
		
		// Adjust the minimal duration of the activity and check consistency
		res = MaxMultiCumulative.adjustMin(tasks(t).getDur, min(up - tasks(t).getLST+1, tasks(t).getECT-low))
		change |= res._1
		if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
	...
	def pruneForbiden(t : Int, r : Int, low : Int, up : Int) : Tuple2[Boolean, CPOutcome] = {
		...
		if (sumHeight - contribution(t) + tasks(t).getMinResource > limits(r)) {
			
			if (tasks(t).getECT > low && tasks(t).getLST <= up && tasks(t).getMinDuration > 0) {
					
				res = MaxMultiCumulative.removeValue(tasks(t).getMachines, r)
				change |= res._1
				if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
				...
				if (tasks(t).getMinDuration > 0) {
					...
					for (i <- low - tasks(t).getMinDuration+1 to up) {
						...
						res = MaxMultiCumulative.removeValue(tasks(t).getStart, i)
						change |= res._1
						if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
					}
					...
					for (i <- low + 1 to up + tasks(t).getMinDuration) {
						...
						res = MaxMultiCumulative.removeValue(tasks(t).getEnd, i)
						change |= res._1
						if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
					}
				}
				...
				res = MaxMultiCumulative.adjustMax(tasks(t).getDur, maxD)
				change |= res._1
				if (res._2 == CPOutcome.Failure) return (change, CPOutcome.Failure)
			}
		}
	...
	def pruneConsumption(t : Int, r : Int, low : Int, up : Int) : Tuple2[Boolean, CPOutcome] = {
		...
		if (tasks(t).getMachines.isBoundTo(r) && tasks(t).getECT > low && tasks(t).getLST <= up && tasks(t).getMinDuration > 0) {
			...
			res = MaxMultiCumulative.adjustMax(tasks(t).getResource, limits(r) - (sumHeight - contribution(t)))
		}
...
object MaxMultiCumulative extends CPModel {
