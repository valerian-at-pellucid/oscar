 ...
 * [-inf,+inf] otherwise
 */
class AbstractLPVar(val solver: AbstractLPSolver, val name: String, val lb: Double, val ub: Double, var unbounded: Boolean) extends Var {
...
class LPConstraint(val solver: AbstractLPSolver, val cstr: LinearConstraint, val index: Int, val name: String) {
  ...
  private val e = Vector() ++ cstr.linExpr.coef
  private val perm = ((0 until e.size).sortBy(i => e(i)._1.index)).toArray

  val coef: Array[Double] = perm.map(i => e(i)._2)
  val varIds: Array[Int] = perm.map(i => e(i)._1.index)
...
abstract class AbstractLPSolver {
  ...
  protected var cons: List[LPConstraint] = List()
  protected var vars: List[AbstractLPVar] = List()
  var varsSize = 0
  var consSize = 0
  ...
  def add(constr: LinearConstraint, name: Option[String] = None): LPConstraint = {
    val cstName = name match {
      case Some(n) => n
      case None => "cstr" + consSize
    }
    val constraint = new LPConstraint(this, constr, consSize, cstName)
  ...
  def addAllConstraints() {
    var nbC = 0
    cons foreach { c: LPConstraint =>
      if (nbC % 1000 == 0) println("Added " + nbC + " constraints.")
  ...
  def subjectTo(constraintsBlock: => Unit) {
    ...
    solver.startModelBuilding(consSize, varsSize)
  ...
  def solveModel() {
    ...
    if ((statuss == LPStatus.OPTIMAL) || (statuss == LPStatus.SUBOPTIMAL)) {
      (0 until varsSize) foreach { i => solution(i) = solver.getValue(i) }
    }
  }
  ...
  def checkConstraints(tol: Double = 10e-6): Boolean = {
    var violation = false
    // added because we use a list for vars
    val varsVec = Vector() ++ vars
    cons foreach {
      case c => //case (i,c) =>
        ...
        for ((i, a) <- c.varIds.zip(c.coef)) {
          if (i < varsSize) {
            val x = varsVec(i)
            res += a * x.getValue
          } else throw new IllegalArgumentException("Variable with index " + i + " not present in lp solver")
