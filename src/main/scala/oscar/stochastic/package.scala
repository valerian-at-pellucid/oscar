...
package object stochastic {
  ...
  implicit def opd = DoubleOp
  implicit def opi = IntOp
  implicit def opl = LongOp

  implicit def toOperationablei(i: Int) = new IntOp(i)
  implicit def toOperationabled(i: Double) = new DoubleOp(i)
  implicit def toOperationablel(i: Long) = new LongOp(i)
